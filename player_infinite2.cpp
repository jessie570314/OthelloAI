...
void State::setHeuristic(int type){
    ...
    corner+=cornerzone;
    //if(mobility[3-type]==0)mobility[type]*=10;
    ...
    else heuristic=(w-n)+3*(mobility[type]-mobility[3-type])+5*num+7*(stability[type]-stability[3-type]);
};
...
void write_valid_spot(std::ofstream& fout) {
    State a=State(board);
    Point ans=search(1,a,player,0,100000,fout);
    fout << ans.x << " " << ans.y << std::endl;
    fout.flush();
    ans=search(4,a,player,0,100000,fout);
    fout << ans.x << " " << ans.y << std::endl;
    fout.flush();
